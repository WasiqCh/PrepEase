...
const TeacherLayout = () => {
  ...
  const user = userStr ? JSON.parse(userStr) : { email: "Teacher" };
    ...
    <div className="flex min-h-screen bg-[#FDFBF7] font-sans">
      ...
      <aside className="fixed inset-y-0 left-0 z-50 w-64 bg-white text-stone-600 shadow-sm flex flex-col border-r border-stone-200">
        <div className="flex flex-col items-start px-6 py-8 border-b border-stone-200">
          ...
          <p className="font-mono text-[10px] uppercase tracking-widest font-bold text-stone-400">Teacher Portal</p>
        </div>
        ...
        <nav className="flex-1 px-4 py-6 overflow-y-auto">
          ...
          <div className="font-mono text-[10px] uppercase tracking-widest px-4 mt-6 mb-2 text-stone-400 font-bold">Content</div>
          <Link to="/teacher/lectures" className={navItemClass('/teacher/lectures')}>
            <UploadCloud size={18} strokeWidth={1.5} /> Upload Lectures
          </Link>
          <Link to="/teacher/assignments" className={navItemClass('/teacher/assignments')}>
            <FileText size={18} strokeWidth={1.5} /> Assignments
          </Link>
          <Link to="/teacher/quizzes" className={navItemClass('/teacher/quizzes')}>
            <HelpCircle size={18} strokeWidth={1.5} /> Quizzes
          </Link>
          <div className="font-mono text-[10px] uppercase tracking-widest px-4 mt-6 mb-2 text-stone-400 font-bold">Monitoring</div>
          <Link to="/teacher/students" className={navItemClass('/teacher/students')}>
            <Users size={18} strokeWidth={1.5} /> Student Progress
          </Link>
          ...
          <div className="px-6 py-4 border-b border-stone-200">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col min-w-0 flex-1">
                ...
                <span className="font-mono text-[10px] uppercase tracking-widest text-stone-400 font-bold">Teacher</span>
              </div>
            </div>
          </div>
          <div className="p-4">
